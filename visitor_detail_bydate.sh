...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/visitor_detail_bydate
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}visitor_detail_bydate"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}visitor_detail_bydate
...
FROM (
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}visitor_detail_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
