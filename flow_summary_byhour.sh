...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/flow_summary_byhour
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}flow_summary_byhour"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}flow_summary_byhour
...
FROM (
	SELECT stat_hour, lib, project_name, count(t1.pv) AS pv
		, COUNTDistinct(t1.user) AS uv, COUNTDistinct(t1.client_ip) AS ipCount
	FROM (
		SELECT stat_hour,lib, project_name
			, multiIf(lib = 'js'
	...
	GROUP BY stat_hour,lib, project_name WITH CUBE
) t2
	LEFT JOIN (
		SELECT stat_hour,lib, project_name, count(1) AS visitCount
			, sum(diff) AS visitTime,sum(multiIf(pv=1 ,1,0)) as bounce
		FROM (
			SELECT lib, stat_hour,project_name, arraySort(groupUniqArray(stat_hour)) AS stat_hours
				, max(log_time) - min(log_time) AS diff
				, count(1) AS pv
			FROM ${ck_log_db}log_analysis
			WHERE stat_date = '${cal_date}' 
				AND event_session_id <> ''
			GROUP BY event_session_id,  stat_hour,lib, project_name
		) t3
		WHERE indexOf(stat_hours, stat_hour) = 1 
		GROUP BY stat_hour,lib, project_name WITH CUBE
	) t4
	ON t2.stat_hour=t4.stat_hour AND t2.lib = t4.lib
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}flow_summary_byhour FINAL SETTINGS optimize_skip_merged_partitions=1"
