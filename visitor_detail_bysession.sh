...
# 文件存放目录
dir_path=$CLKLOG_SCRIPT_LOG/visitor_detail_bysession
mkdir -p ${dir_path}
...
SELECT '${cal_date}' AS stat_date
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}visitor_detail_bysession FINAL SETTINGS optimize_skip_merged_partitions=1"
