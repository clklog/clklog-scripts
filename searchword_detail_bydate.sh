...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/searchword_detail_bydate
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}searchword_detail_bydate"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}searchword_detail_bydate
...
FROM (
	SELECT lib, project_name, is_first_day, country, province,latest_search_keyword
	...
	ON t2.lib = t4.lib
		...
		AND t2.is_first_day = t4.is_first_day and t2.latest_search_keyword=t4.latest_search_keyword"
   echo ${cur_date_sql}

  ${ck_login} --query="${cur_date_sql}"
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}searchword_detail_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
