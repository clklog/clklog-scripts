...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/visitor_summary_bydate
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}visitor_summary_bydate"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}visitor_summary_bydate
SELECT '${cal_date}' AS stat_date, If(t2.is_first_day = '', 'all', t2.is_first_day) AS is_first_day
	, If(t2.lib = '', 'all', t2.lib) AS lib
	, multiIf(t2.project_name = '', 'all', t2.project_name = 'N/A', '', t2.project_name) AS project_name
	, t2.pv, t4.visitCount, t2.uv, t2.ipCount, t4.visitTime
	, t4.bounce, NOW() AS update_time
FROM (
	SELECT lib, project_name, is_first_day,count(t1.pv) AS pv
		, COUNTDistinct(t1.user) AS uv, COUNTDistinct(t1.client_ip) AS ipCount
	FROM (
	...
	GROUP BY lib, project_name,is_first_day WITH CUBE
) t2
	LEFT JOIN (
		SELECT lib, project_name,is_first_day, count(1) AS visitCount
			, sum(diff) AS visitTime,sum(multiIf(pv=1 ,1,0)) as bounce
		FROM (
			SELECT lib, project_name,is_first_day, arraySort(groupUniqArray(stat_date)) AS stat_dates
			...
			GROUP BY event_session_id,is_first_day, lib, project_name
		) t3
		WHERE indexOf(stat_dates, toDate('${cal_date}')) = 1
		GROUP BY lib, project_name,is_first_day WITH CUBE
	) t4
	ON t2.is_first_day=t4.is_first_day AND t2.lib = t4.lib
		AND t2.project_name = t4.project_name"
  echo ${cur_date_sql}

  ${ck_login} --query="${cur_date_sql}"
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}visitor_summary_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
