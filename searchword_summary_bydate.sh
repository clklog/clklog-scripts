...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/searchword_summary_bydate
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}searchword_summary_bydate"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}searchword_summary_bydate
...
FROM (
	SELECT lib, project_name, latest_search_keyword,count(t1.pv) AS pv 
	FROM (
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}searchword_summary_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
