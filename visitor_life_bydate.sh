...
# 文件存放目录
dir_path=$CLKLOG_SCRIPT_LOG/visitor_life_bydate
mkdir -p ${dir_path}
...
FROM (
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}visitor_life_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
