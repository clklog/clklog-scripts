...
SELECT '${cal_date}' AS stat_date
	...
	, multiIf(t2.url = '', 'all', t2.url = 'N/A', '直接访问', t2.url) AS uri
...
FROM (
	SELECT lib, project_name, is_first_day, country, province,url
	...
	GROUP BY lib, project_name, is_first_day, country, province,url WITH CUBE
) t2
	LEFT JOIN (
		SELECT lib, project_name, is_first_day, country, province,url
		...
		FROM (
			...
			GROUP BY event_session_id, lib, project_name, is_first_day, country, province,url
		) t3
	...
	ON t2.lib = t4.lib
		...
		AND t2.is_first_day = t4.is_first_day and t2.url=t4.url
   LEFT JOIN (
		SELECT 
			t_e_c3.lib AS lib,
			...
			(
				...
				GROUP BY t_e_c2.lib, t_e_c2.project_name, t_e_c2.is_first_day, t_e_c2.country, t_e_c2.province,t_e_c1.event_session_id,t_e_c1.url
			) t_e_c3
		    GROUP BY t_e_c3.lib, t_e_c3.project_name, t_e_c3.is_first_day, t_e_c3.country, t_e_c3.province,t_e_c3.url WITH CUBE
	) t5
	ON t2.lib = t5.lib
		AND t2.project_name = t5.project_name
		AND t2.country = t5.country
		AND t2.province = t5.province
		AND t2.is_first_day = t5.is_first_day and t2.url=t5.url
	LEFT JOIN (
		SELECT 
			t_e_c3.lib AS lib,
			t_e_c3.project_name AS project_name,
	...
	ON t2.lib = t6.lib
		...
		AND t2.is_first_day = t6.is_first_day and t2.url=t6.url
	LEFT JOIN (
		SELECT 
			t_e_c3.lib AS lib,
			t_e_c3.project_name AS project_name,
	...
	ON t2.lib = t7.lib
		...
		AND t2.is_first_day = t7.is_first_day and t2.url=t7.url"
   echo ${cur_date_sql}
