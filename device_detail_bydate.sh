...
# 文件存放目录
dir_path=$CLKLOG_SCRIPT_LOG/device_detail_bydate
mkdir -p ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}device_detail_bydate"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}device_detail_bydate
SELECT '${cal_date}' AS stat_date
	...
	, multiIf(t2.manufacturer = '', 'all', t2.manufacturer = 'N/A', '', t2.manufacturer) AS manufacturer
	, t2.pv, t4.visitCount, t2.uv, t2.new_uv, t2.ipCount
	, t4.visitTime, t4.bounce,NOW() AS update_time
FROM (
	SELECT lib, project_name, is_first_day, country, province,manufacturer
	...
	GROUP BY lib, project_name, is_first_day, country, province,manufacturer WITH CUBE
) t2
	LEFT JOIN (
		SELECT lib, project_name, is_first_day, country, province,manufacturer
		...
		FROM (
			...
			GROUP BY event_session_id, lib, project_name, is_first_day, country, province,manufacturer
		) t3
	...
	ON t2.lib = t4.lib
		...
		AND t2.is_first_day = t4.is_first_day and t2.manufacturer=t4.manufacturer"
   echo ${cur_date_sql}

  ${ck_login} --query="${cur_date_sql}"
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}device_detail_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
