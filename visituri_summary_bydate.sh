...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/visituri_summary_bydate
mkdir ${dir_path}
...
SELECT '${cal_date}' AS stat_date
	...
	,multiIf(t2.url = '', 'all', t2.url = 'N/A', '', t2.url) AS url, t2.pv,  NOW() AS update_time
FROM (
	...
	GROUP BY lib, project_name,url WITH CUBE
) t2"
  # echo ${cur_date_sql}

  ${ck_login} --query="${cur_date_sql}"
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}visituri_summary_bydate FINAL"
