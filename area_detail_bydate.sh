...
# 文件存放目录
dir_path=$CLKLOG_SCRIPT_LOG/area_detail_bydate
mkdir -p ${dir_path}
...
FROM (
	...
	GROUP BY lib, project_name, is_first_day, country, province,city WITH CUBE
) t2
	LEFT JOIN (
		SELECT lib, project_name, is_first_day, country, province,city
		...
		FROM (
			...
			GROUP BY event_session_id, lib, project_name, is_first_day, country, province,city 
		) t3
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}area_detail_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
