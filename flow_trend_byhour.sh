...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/flow_trend_byhour
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}flow_trend_byhour"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}flow_trend_byhour
	...
	LEFT JOIN (
		...
		FROM (
			SELECT stat_hour,lib, project_name, is_first_day
				...
				, count(1) AS pv, arraySort(groupUniqArray(stat_hour)) AS stat_hours
			FROM ${ck_log_db}log_analysis
			WHERE stat_date <= '${cal_date}'
				AND stat_date >= '${previous_date}'
				AND event_session_id <> ''
			GROUP BY event_session_id,stat_hour, lib, project_name, is_first_day, country, province
		) t3
		WHERE indexOf(stat_dates, toDate('${cal_date}')) = 1 and indexOf(stat_hours,stat_hour) = 1
		GROUP BY stat_hour,lib, project_name, is_first_day, country, province WITH CUBE
	) t4
	ON t2.lib = t4.lib
		...
		AND t2.is_first_day = t4.is_first_day and t2.stat_hour=t4.stat_hour  WHERE t2.stat_hour <> ''"
  echo ${cur_date_sql}

  ${ck_login} --query="${cur_date_sql}"
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}flow_trend_byhour FINAL SETTINGS optimize_skip_merged_partitions=1"
