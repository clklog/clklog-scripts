...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/flow_summary_bydate
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}flow_summary_bydate"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}flow_summary_bydate
...
FROM (
	SELECT lib, project_name, count(t1.pv) AS pv
	...
	GROUP BY lib, project_name WITH CUBE
) t2
	LEFT JOIN (
		SELECT lib, project_name, count(1) AS visitCount
			, sum(diff) AS visitTime,sum(multiIf(pv=1 ,1,0)) as bounce
		FROM (
			SELECT lib, project_name, arraySort(groupUniqArray(stat_date)) AS stat_dates
			...
			GROUP BY event_session_id, lib, project_name
		) t3
  ...
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}flow_summary_bydate FINAL SETTINGS optimize_skip_merged_partitions=1"
