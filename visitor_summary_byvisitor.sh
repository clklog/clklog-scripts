...
# 文件存放目录
dir_path=/usr/local/services/scripts/cklog/visitor_summary_byvisitor
mkdir ${dir_path}
...
if [ $1 -eq 0 ]; then
  ...
  ${ck_login} --query="truncate table ${ck_stat_db}visitor_summary_byvisitor"
...
do
  ...
  cur_date_sql="INSERT INTO ${ck_stat_db}visitor_summary_byvisitor
SELECT '${cal_date}' AS stat_date
	...
	, t2.pv, t4.visitCount, t2.uv, t2.new_uv, t2.ipCount, t4.visitTime
	, t4.bounce
	,t2.latest_time as latest_time
	,NOW() AS update_time
FROM (
	SELECT lib, project_name, is_first_day, country, province,distinct_id
		, count(t1.pv) AS pv, COUNTDistinct(t1.user) AS uv
		, COUNTDistinct(t1.new_user) AS new_uv, COUNTDistinct(t1.client_ip) AS ipCount,max(t1.log_time) as latest_time 
	FROM (
	...
	LEFT JOIN (
		...
		FROM (
			...
			GROUP BY event_session_id, lib, project_name, is_first_day, country, province,distinct_id
		) t3
	...
	ON t2.lib = t4.lib
		...
		WHERE t2.distinct_id <> ''"
   echo ${cur_date_sql}

  ${ck_login} --query="${cur_date_sql}"
  # 强制进行分区合并，实现重复数据的删除
  ${ck_login} --query="optimize table ${ck_stat_db}visitor_summary_byvisitor FINAL SETTINGS optimize_skip_merged_partitions=1"
